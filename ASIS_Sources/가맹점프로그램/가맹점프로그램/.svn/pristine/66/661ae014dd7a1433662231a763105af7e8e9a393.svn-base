...
Private Sub Form_Load()
    ...
    SmartUpdateX.IniFileName = "update.txt"                       ' 대소문자를 정확히 해야합니다.. !!
End Sub

Public Sub FileDown_KSNet_Dongle_Cab()
    ...
    temp = SmartUpdateX.ReadInfo("DOWN", "KSNetDongleCab") '원격지원 프로그램 다운로드
    ...
    If Trim(temp) <> "" Then
        ...
        If InStr(FileName, ".exe") > 0 Then FileName = Replace(FileName, ".exe", ".OCX")
...
Public Sub FileDown_KSNet_Dongle_Ocx()
    Dim FilePath  As String
    Dim FileName  As String
    Dim dtemp
    ...
    temp = SmartUpdateX.ReadInfo("DOWN", "KSNetDongleOcx") '원격지원 프로그램 다운로드
...
Public Sub AidSupportFileDown()
    ...
    temp = SmartUpdateX.ReadInfo("DOWN", "AIDSUPPORT") '원격지원 프로그램 다운로드
...
Public Sub DBUpdate_FileDown()
    ...
    temp = SmartUpdateX.ReadInfo("DOWN", "DBUPDATEFILE")
...
Public Sub DataBaseCreate_FileDown()
    Dim FilePath  As String
    Dim FileName  As String
    Dim dtemp
    ...
    temp = SmartUpdateX.ReadInfo("DOWN", "FILE2")
    'temp = SmartUpdateX.ReadInfo("DOWN", "DBUPDATEFILE") '자동업데이트 프로그램 다운로드
...
Public Sub CleanAidHelp_FileDown()
    ...
    temp = SmartUpdateX.ReadInfo("DOWN", "AIDHELPFILE")
...
Public Sub SmartUpdate()
    ...
    temp = SmartUpdateX.ReadInfo("DOWN", "FILE1")
  
    If Trim(temp) <> "" Then
        lblFile.Caption = temp
        
        On Error Resume Next
        Kill App.Path + "\" & "CleanAID_OLD.old"
        On Error GoTo 0
        ...
        If Dir(App.Path & "\update.bat") = "" Then
            ...
            Print #1, "ren CleanAID.exe CleanAid_Old.old"
            Close FileNo
        End If
        
        Call Shell(App.Path + "\" & "Update.BAT", vbNormalNoFocus)
        ...
        If i = 1 Then '' 수신 성공
            ...
            On Error Resume Next
            FileCopy SmartUpdateX.WorkDir + updatetemp, App.Path & "\" & "CleanAID.exe"
            Kill App.Path & "\update.bat"
            On Error GoTo 0
...
Private Sub SmartUpdateX_OnWork(ByVal Url As String, ByVal WorkCount As Long, StopSW As Boolean)
    ...
    Debug.Print "OnWork(; [" + Url + "]; 크기; [" + Str(WorkCount) + "]; """
...
Private Sub SmartUpdateX_OnWorkBegin(ByVal Url As String, ByVal WorkCountMax As Long, StopSW As Boolean)
    ...
    Debug.Print "WorkBegin[" + Url + "]받을 화일크기[" + Str(WorkCountMax) + "]"
...
Private Sub SmartUpdateX_OnWorkEnd(ByVal Url As String)
    ...
    Debug.Print "WorkEnd[" + Url + "] 종료"
End Sub
 
Public Sub Download_OCX(TagName As String)
    ...
    temp = SmartUpdateX.ReadInfo("OCX", TagName)
    'temp = SmartUpdateX.ReadInfo("DOWN", "DBUPDATEFILE") '자동업데이트 프로그램 다운로드
    ...
    If Trim(temp) <> "" Then
        ...
        If i = 1 Then '성공
            ...
            ttemp = Split(FileName, ".")
            If UCase(ttemp(UBound(ttemp))) = "EXE" Then
                On Error Resume Next
                Kill FilePath + Replace(UCase(FileName), "EXE", "OCX")
                FileCopy FilePath + FileName, FilePath + Replace(UCase(FileName), "EXE", "OCX")
