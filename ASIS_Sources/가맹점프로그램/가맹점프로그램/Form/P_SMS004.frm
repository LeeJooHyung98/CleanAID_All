...
Begin VB.Form P_SMS004 
   ...
   Begin FPSpreadADO.fpSpread fpSpread1 
      ...
      Protect         =   0   'False
      ScrollBars      =   2
      SpreadDesigner  =   "P_SMS004.frx":0000
      VisibleCols     =   2
      VisibleRows     =   50
      AppearanceStyle =   0
   End
        ...
        Case 1
            For lRow = 1 To fpSpread1.MaxRows
                ...
                If fpSpread1.Value = 1 Then
                    fpSpread1.SetText 1, lRow, "0"
                End If
            Next lRow
            
            
            lblSMS(1).Caption = CStr(GetSelectSpread(fpSpread1, 1))
            Exit Sub
            
        Case 0
            For lRow = 1 To fpSpread1.MaxRows
                fpSpread1.Col = 4: fpSpread1.Row = lRow
                
                ' 휴대폰 번호가 있을경우
                If fpSpread1.Text <> "" Then
                    fpSpread1.Col = 10: fpSpread1.Row = lRow
                    ' 해당 회원의 정보를 얻어온다.
                    If Fun_고객정보(fpSpread1.Text) <> "Error" Then
...
Private Sub cmdSendTextSave_Click(Index As Integer)
    ...
    Select Case Index
        ...
        Case 1  ' 수정
            ...
            Query = Query & " WHERE 순번 = '" & Format(cboSendText.ItemData(cboSendText.ListIndex), "00") & "'  "
            ADOCon.Execute Query
...
Private Sub Form_Activate()
    ...
    DTPicker1(0).Value = DateAdd("d", -2, Date)
    DTPicker1(1).Value = DateAdd("d", -2, Date)
...
Private Sub SetUseSMSCount()
    ...
    ADORs.Open Query, m_Host_DataBase, adOpenStatic, adLockOptimistic
    ...
    If Not ADORs.EOF Then
        If ADORs.RecordCount > 0 Then
            lblSMS(0).Caption = Format(ADORs.Fields("잔여수량") & "", "#,##0")
        End If
    End If
    ADORs.Close
    Set ADORs = Nothing
...
Private Sub DataDisplay()
    ...
    fpSpread1.MaxRows = 0: fpSpread1.MaxCols = 8
    fpSpread1.ColWidth(8) = 0
    lblSMS(1).Caption = "0"
    bSSChangeFlag = True
    ...
    '------------------------------------------------------------------------------------
    Query = " SELECT 고객코드, MAX(접수일자) AS 접수일자  "
    Query = Query & "  FROM TB_입출고 "
    Query = Query & " WHERE 접수일자 >= '" & Format(DTPicker1(0).Value, "YYYY-MM-DD") & "' "
    Query = Query & "   AND 접수일자 <= '" & Format(DTPicker1(1).Value, "YYYY-MM-DD") & "' "
    ...
    Do Until ADORs.EOF
        ...
        If Fun_고객정보(ADORs.Fields("고객코드") & "") <> "Error" Then
            ...
            lRow = lRow + 1
            fpSpread1.SetText 2, lRow, 고객정보.성명
            fpSpread1.SetText 3, lRow, 고객정보.전화번호
            fpSpread1.SetText 4, lRow, 고객정보.휴대폰
            fpSpread1.SetText 5, lRow, Format(ADORs.Fields("접수일자") & "", "YYYY-MM-DD")
            fpSpread1.SetFloat 6, lRow, Val(고객정보.미수금)
            fpSpread1.SetText 7, lRow, 고객정보.주소
            fpSpread1.SetText 8, lRow, 고객정보.고객코드
...
Private Function SendSMS() As Boolean
    ...
    ' 최종 확인 메시지
    If MsgBox("메시지를 전송 하시겠습니까?", vbCritical + vbYesNo, "확인") = vbNo Then
        Exit Function
    End If
    ...
    For lRow = 1 To fpSpread1.MaxRows
        ...
        If sFlag = True Then
            ...
            Call fpSpread1.GetText(8, lRow, vTemp):    sValue(7) = CStr(vTemp)
