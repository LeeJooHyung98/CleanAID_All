...
Begin VB.Form frm세탁물인도문자 
   ...
   Begin Threed.SSPanel pnlProg 
      ...
      PictureBackground=   "frm세탁물인도문자.frx":0000
      BorderWidth     =   0
      BevelOuter      =   0
      BevelInner      =   2
   ...
   Begin SSSplitter.SSSplitter SSSplitter1 
      ...
      Begin Threed.SSPanel pnlHeader 
         ...
         Caption         =   "      세탁물 인도 문자"
         PictureBackgroundStyle=   2
         PictureBackground=   "frm세탁물인도문자.frx":02F8
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin Threed.SSPanel SSPanel3 
         ...
         End
         Begin XtremeSuiteControls.PushButton btnSelectPeriod 
            Height          =   690
            Index           =   1
            Left            =   3495
            TabIndex        =   99
         ...
         Begin XtremeSuiteControls.PushButton btnSelectPeriod 
            ...
            Appearance      =   6
         End
         Begin XtremeSuiteControls.PushButton btnSelectUser 
            Height          =   690
            Index           =   0
            Left            =   0
            TabIndex        =   101
      ...
      Begin FPSpreadADO.fpSpread sprDetail 
         ...
         Protect         =   0   'False
         ScrollBars      =   2
         SpreadDesigner  =   "frm세탁물인도문자.frx":2F6E
         VisibleCols     =   2
         VisibleRows     =   50
         HighlightHeaders=   1
         HighlightStyle  =   1
      ...
      Begin XtremeSuiteControls.TabControl TabControl 
         ...
         Begin XtremeSuiteControls.TabControlPage TabControlPage1 
            ...
            Begin XtremeSuiteControls.PushButton cmdSend 
               ...
               Caption         =   " 문자보내기"
               Appearance      =   6
               Picture         =   "frm세탁물인도문자.frx":1181E
            End
            Begin VB.Label Label2 
               AutoSize        =   -1  'True
               BackStyle       =   0  '투명
               Caption         =   $"frm세탁물인도문자.frx":11F18
               BeginProperty Font 
                  Name            =   "굴림체"
                  Size            =   9
                  Charset         =   129
      ...
      Begin FPSpreadADO.fpSpread sprGrid 
         ...
         MaxRows         =   300
         MoveActiveOnFocus=   0   'False
         SpreadDesigner  =   "frm세탁물인도문자.frx":4D2B9
         VisibleCols     =   2
         VisibleRows     =   50
         HighlightHeaders=   1
         HighlightStyle  =   1
   ...
   Begin Threed.SSPanel SSPanel_SMS 
      ...
      Begin VB.Label Label1 
         Caption         =   "※ 입금 계좌: 국민은행 299601-01-067243 (주)크린에이드"
         ForeColor       =   &H000000FF&
         Height          =   285
         Index           =   0
         Left            =   60
...
Private Sub btnSelectPeriod_Click(Index As Integer)
    Dim LoopI As Integer
    For LoopI = 0 To btnSelectPeriod.UBound
        btnSelectPeriod(LoopI).Checked = False
        btnSelectPeriod(LoopI).Font.Bold = False
    Next LoopI
    btnSelectPeriod(Index).Checked = True
    btnSelectPeriod(Index).Font.Bold = True
    dtpDay(0).Visible = False
    dtpDay(1).Visible = False
    dtpDay(1).Value = Now
    Select Case Index
        Case 0
            dtpDay(0).Value = DateAdd("d", 0, Date)
        Case 1
            dtpDay(0).Value = DateAdd("d", -7, Date)
        Case 2
            dtpDay(0).Value = DateAdd("m", -1, Date)
        Case 3
            dtpDay(0).Value = DateAdd("m", -3, Date)
        Case 4
            dtpDay(0).Visible = True
            dtpDay(1).Visible = True
    End Select
    sprGrid.MaxRows = 0
    sprDetail.MaxRows = 0
End Sub

Private Sub btnSelectUser_Click(Index As Integer)
    Dim LoopI As Integer
    For LoopI = 0 To btnSelectUser.UBound
        btnSelectUser(LoopI).Checked = False
        btnSelectUser(LoopI).Font.Bold = False
    Next LoopI
    btnSelectUser(Index).Checked = True
    btnSelectUser(Index).Font.Bold = True
...
Private Sub chk_KAKAO_Click()

    Call SetIniStr("KAKAO", "ALLIM_ENABLE", CStr(chk_KaKao.Value), iniFile)
...
Private Sub cmdSendTextSave_Click(Index As Integer)
    ...
    Select Case Index
        Case 0 ' 추가
            ...
            If Int(iCode) > 9 Then
                MsgBox "9개 이상은 저장할 수 없습니다.", vbInformation, "확인"
                Exit Sub
            End If
...
Private Sub Form_Load()
    ...
    dtpDay(1).Value = Date
    dtpDay(2).Value = DateAdd("d", -1, Date)
    ...
    If cmdBtn(4).Enabled = False Then txtSMS.Locked = True
    cmdChange.Enabled = Not cmdBtn(4).Enabled
    
    TabControl.SelectedItem = 0
    chk_KaKao.Value = GetIniStr("KAKAO", "ALLIM_ENABLE", "1", iniFile)
    Call btnSelectUser_Click(0)
    Call btnSelectPeriod_Click(4)
...
Private Sub Form_Resize()
    ...
    cmdList.Left = cmdBtn(5).Left - cmdList.Width - 20
    
    txtCount(1).Left = SSPanel(0).Width - txtCount(1).Width - 50
    Label2(5).Left = txtCount(1).Left - 870
End Sub
...
Private Sub txtSend2_Change()
    ...
    If LenB(StrConv(txtSend2.Text, vbFromUnicode)) > m_SMS_Lng Then
        ...
        MsgBox "작성된 메시지가 " & CStr(m_SMS_Lng) & "자 이상 입니다. " & m_SMS_Lng & "자 이상은 전송할 수 없습니다.", vbCritical, "확인"
        Exit Sub
    Else
...
ErrRtn:
    ...
    Call Error_Msg("", "SetUseSMSCount", Err.Number, Err.description)
End Sub
...
'    Query = Query & "        AND (bb.출고일자 = '' OR bb.출고일자 IS NULL)"
'    Query = Query & ""
...
'    Query = Query & "   AND (aa.가맹점입고일자 <> '' OR aa.가맹점입고일자 IS NOT NULL)"
...
'    Query = Query & " Having Count(bb.고객코드) = 0"
'
    If btnSelectUser(0).Checked = True Then
        Query = "EXEC SP_SMS_VIEW1 '" & Format(dtpDay(0).Value, "YYYY-MM-DD") & "', '" & Format(dtpDay(1).Value, "YYYY-MM-DD") & "'"
    Else
        Query = ""
        Query = Query & " SELECT " & vbCrLf
        Query = Query & "   a.고객코드, 성명, 전화번호, 휴대전화, '' as 문자발송여부, MIN(TagS) as TagS, MAX(TagE) as TagE, SUM(TagCnt) as TagCnt, SUM(ReturnCnt) as ReturnCnt" & vbCrLf
        Query = Query & " FROM" & vbCrLf
        Query = Query & " (" & vbCrLf
        Query = Query & "   SELECT 접수번호, 고객코드, SUM(success) as success, SUM(remain) as remain, SUM(ReturnCnt) as ReturnCnt, MIN(택번호) as TagS, MAX(택번호) as TagE, COUNT(*) as TagCnt FROM (" & vbCrLf
        Query = Query & "       SELECT " & vbCrLf
        Query = Query & "           접수번호, " & vbCrLf
        Query = Query & "           고객코드," & vbCrLf
        Query = Query & "           CASE 가맹점입고일자 " & vbCrLf
        Query = Query & "           WHEN '' THEN 0" & vbCrLf
        Query = Query & "           ELSE 1 " & vbCrLf
        Query = Query & "           END as success, " & vbCrLf
        Query = Query & "           CASE 가맹점입고일자 " & vbCrLf
        Query = Query & "           WHEN '' THEN 1 " & vbCrLf
        Query = Query & "           ELSE 0" & vbCrLf
        Query = Query & "           END as remain ," & vbCrLf
        Query = Query & "           CASE 지사출고상태" & vbCrLf
        Query = Query & "           WHEN '2' THEN 1" & vbCrLf
        Query = Query & "           ELSE 0" & vbCrLf
        Query = Query & "           END as ReturnCnt," & vbCrLf
        Query = Query & "           택번호" & vbCrLf
        Query = Query & "       FROM tb_입출고 "
        Query = Query & "       WHERE "
        Query = Query & "       가맹점입고일자 between '" & Format(dtpDay(0).Value, "YYYY-MM-DD") & " 00:00:00' and '" & Format(dtpDay(1).Value, "YYYY-MM-DD") & " 23:59:59' and 출고일자 = '' AND 판매취소 <> 'Y'"
        Query = Query & "       AND 접수번호 IN (" & vbCrLf
        Query = Query & "           SELECT 접수번호 " & vbCrLf
        Query = Query & "           FROM tb_입출고 " & vbCrLf
        Query = Query & "           WHERE 가맹점입고일자 between '" & Format(dtpDay(0).Value, "YYYY-MM-DD") & " 00:00:00' and '" & Format(dtpDay(1).Value, "YYYY-MM-DD") & " 23:59:59' and 출고일자 = ''" & vbCrLf
        Query = Query & "           GROUP BY 접수번호" & vbCrLf
        Query = Query & "       ) " & vbCrLf
        Query = Query & "   ) a" & vbCrLf
        Query = Query & "   GROUP BY 접수번호,고객코드" & vbCrLf
        Query = Query & " ) a LEFT join tb_고객정보 b ON a.고객코드 = b.고객코드" & vbCrLf
        Query = Query & " WHERE remain = 0" & vbCrLf
        Query = Query & " GROUP BY a.고객코드,성명, 전화번호, 휴대전화" & vbCrLf
    End If
    Set ADORs = New ADODB.RecordSet
    ADORs.CursorLocation = adUseClient
    ADORs.Open Query, ADOCon, adOpenStatic, adLockBatchOptimistic, adCmdText
...
Private Sub Data_Display_20150703()
    ...
    Query = Query & "   AND (가맹점입고일자 <> '' OR 가맹점입고일자 IS NOT NULL)"
    Query = Query & "   AND (출고일자 = '' OR 출고일자 IS NULL)"
    Query = Query & "   AND (판매취소 <> 'Y')"
        ...
        Do Until ADORs.EOF
            ...
            If SUBRs.EOF Then
                ...
                If Get_고객정보(ADORs!고객코드) <> "Error" Then
                    ...
                    Query = Query & "   AND (가맹점입고일자 <> '')"                                        ' 본사 출고 상태인것
                    Query = Query & "   AND (판매취소 <> 'Y')"
                    Query = Query & "   AND (출고일자 = '' OR 출고일자 IS NULL)"
                    Set SUBRs = New ADODB.RecordSet
                    SUBRs.Open Query, ADOCon, adOpenForwardOnly, adLockReadOnly
                    ...
                    If Not SUBRs.EOF Then
                        ...
                        Else
                            If sLastDate <> "연결오류" Then
                                sLastDate = GetLastUseDate(고객정보.휴대전화) '-- 시간이 너무 걸리네 ㅡㅡ
                            End If
                    ...
                    '-----------------------------------------------------------------------------------
                    ' 반품수량
                    FindCount = Get_반품수량(Format(dtpDay(0).Value, "YYYY-MM-DD"), Format(dtpDay(1).Value, "YYYY-MM-DD"), Trim(ADORs!고객코드))
                    .Col = 8: .Text = CStr(FindCount)
...
Private Function SendSMS() As Boolean
    ...
    ' 광고 내용 확인
    If InStr(txtSMS.Text, "세일") > 0 Or InStr(txtSMS.Text, "행사") > 0 Or InStr(txtSMS.Text, "할인") > 0 Or InStr(txtSMS.Text, "광고") > 0 Then
        sMsg = "정보 통신망 이용 촉진 및 정보 보호 등에 관한 법률로 인한 안내" & vbNewLine
        sMsg = sMsg & vbNewLine
        sMsg = sMsg & " - 해당 화면에서는 세일, 광고, 할인, 행사와 관련된 문자를 발송할 수 없습니다." & vbNewLine
...
'            End If
            ...
            If chk_KaKao.Value = 1 Then
                Call send_Kakao_Delivery(sValue(7), sValue(2), txtSMS.Text)
            Else
                Call send_Purio_SMS(sValue(7), sValue(2), txtSMS.Text, "1")
            End If
            nSendCount = nSendCount + 1
        End If
        sprGrid.Col = 1: sprGrid.Text = 0
    Next lRow
...
ErrRtn:
    ...
    Call Error_Msg("", "SendSMS", Err.Number, Err.description)
End Function
...
Private Sub DisplayDetailData(ByVal sCustCode As String)
    ...
    Query = Query & " WHERE " '(접수일자 >= '" & Format(dtpDay(0).Value, "YYYY-MM-DD") & "'"
    If btnSelectUser(0).Checked = True Then
        Query = Query & "   (접수일자 >= '" & Format(dtpDay(0).Value, "YYYY-MM-DD") & "' AND  접수일자 <= '" & Format(dtpDay(1).Value, "YYYY-MM-DD") & "')"
        Query = Query & "   AND 고객코드   = '" & sCustCode & "'"
    Else
        Query = Query & "   (가맹점입고일자 >= '" & Format(dtpDay(0).Value, "YYYY-MM-DD") & "' AND  가맹점입고일자 <= '" & Format(dtpDay(1).Value, "YYYY-MM-DD") & "')"
        Query = Query & "   AND 고객코드   = '" & sCustCode & "'"
    End If
...
Private Function GetLastUseDate(ByVal CustPhone As String) As String
    ...
    'Call Error_Msg2("GetLastUseDate", "Query", "", Query)
