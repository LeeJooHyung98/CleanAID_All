...
'    FPrtBottom.Account0 = frmÁ¢¼ö°áÁ¦.Label6
'    FPrtBottom.Account1 = frmÁ¢¼ö°áÁ¦.txtMoney.Text
'    FPrtBottom.Account2 = frmÁ¢¼ö°áÁ¦.Label3
'
'
'    On Error GoTo printError
'    ' Ãâ·Â ÇüÅÂ Á¤ÀÇ
...
Public Sub FormPrint()
    ...
    For iCnt = 1 To iPage
        ...
        If iCnt = iPage Then
            ...
            temp = temp & vbCr & vbLf & vbLf & "        TEL:" & ´ë¸®Á¡Á¤º¸.ÀüÈ­¹øÈ£
            
            Print #1, temp
            'temp = "        " & frmMain.StatusBar1.Panels(2) & "   " & iCnt & "/" & iPage
            temp = "        " & ´ë¸®Á¡Á¤º¸.´ë¸®Á¡¸í & "   " & iCnt & "/" & iPage
            Print #1, temp
        Else
            '´ë¸®Á¡¸í
            'temp = vbLf & vbLf & vbLf & vbCr & vbLf & "        TEL:" & frmMain.StatusBar1.Panels(5)
            temp = vbLf & vbLf & vbLf & vbCr & vbLf & "        TEL:" & ´ë¸®Á¡Á¤º¸.ÀüÈ­¹øÈ£
...
Function GetPrtGubun() As Integer
    ...
    Query = "SELECT ÇÁ¸°ÅÍ FROM TB_´ë¸®Á¡Á¤º¸"
...
Function GetPrtBOGubun() As Integer
    ...
    Query = "SELECT º¸°üÁõÁ¾·ù FROM TB_´ë¸®Á¡Á¤º¸"
    Set Rs = New ADODB.Recordset
    Rs.Open Query, ADOCon, adOpenForwardOnly, adLockReadOnly
...
'    End If
    ...
    Call Receipt_Init 'ÀüÃ¼ Ãâ·Â °¹¼ö¹× Ãâ·Â ³»¿ë º¯¼ö¿¡ ÃÊ±âÈ­ 'GoSub Print_Value_Init
...
Print_Next:
    ...
    Do Until SUBRs.EOF
        If Fun_°í°´Á¤º¸(SUBRs!°í°´ÄÚµå) = "Error" Then
...
Print_Bottom:
    ...
    Query = Query & " WHERE SUBSTRING(È¯ºÒÀÏÀÚ,1,10) = '" & prtDay & "'"
    Set Rs = New ADODB.Recordset
    Rs.Open Query, ADOCon, adOpenStatic, adLockOptimistic
...
'   hhh$(49) = "                   ¹«·á¼¼Å¹±Ç¼ö: #,##0 Àå   °í°´¼ö: #,##0 ¸í  ¼¼Æ®ÇÒÀÎ±Ý¾×: ###,##0 ¿ø ¿¡´©¸®ÇÒÀÎ±Ý¾×: ###,##0 ¿ø "
    ...
    Query = Query & " FROM TB_ÀÔÃâ°í "
    Query = Query & " WHERE SUBSTRING(¼¼Å¹ºñÈ¯ºÒÀÏÀÚ,1,10) = '" & prtDay & "' "
    Query = Query & " ORDER BY ÅÃ¹øÈ£ "
    Set SUBRs = New ADODB.Recordset
    SUBRs.Open Query, ADOCon, adOpenStatic, adLockOptimistic
    
    Do Until SUBRs.EOF
        Call Fun_°í°´Á¤º¸(SUBRs!°í°´ÄÚµå & "")
        
        If i = 0 Then
            TextData(1) = CStr(dblSaleReturnCnt)
            TextData(2) = Format(SUBRs!Á¢¼öÀÏÀÚ, "YYYY-MM-DD") & "  " & Format(Replace(SUBRs!ÅÃ¹øÈ£, "-", ""), "@-@@@") & "  " & °í°´Á¤º¸.¼º¸í & " ( " & °í°´Á¤º¸.ÈÞ´ëÆù & " )"
            
            Print #FHandle, Line_Format(hhh$(45), TextData(), 2)
        Else
            TextData(1) = Format(SUBRs!Á¢¼öÀÏÀÚ, "YYYY-MM-DD") & "  " & Format(Replace(SUBRs!ÅÃ¹øÈ£, "-", ""), "@-@@@") & "  " & °í°´Á¤º¸.¼º¸í & " ( " & °í°´Á¤º¸.ÈÞ´ëÆù & " )"
    ...
    If Format(Date, "YYYY-MM-DD") <= "20090831" And ´ë¸®Á¡Á¤º¸.MasterCode <> M_COUPON_KLENZ_CODE Then
...
Public Function PrintSagoReport(cdPrt As CommonDialog, prtNum As String) As Boolean
    ...
    Query = " SELECT * FROM TB_»ç°íÇ° WHERE ÀÏ·Ã¹øÈ£ = " & Val(prtNum) & " "
    Set SUBRs = New ADODB.Recordset
    SUBRs.Open Query, ADOCon, adOpenStatic, adLockOptimistic
...
'    Query = "SELECT DISTINCT P.Á¢¼öÀÏÀÚ, (P1.ÀüÈ­¹øÈ£+'-'+ P1.ÀüÈ­2)  AS ÀüÈ­¹øÈ£ , P1.¼º¸í, P.Ç°¸í, "
...
Public Function GetGoodsName(ByVal sCode As String) As String
    ...
    Query = "SELECT Ç°¸í FROM TB_ÂüÁ¶ÄÚµå "
...
Private Function QN_Day_Info(ByVal sDate As String, ByRef dCleanTotal As Double, ByRef dBonSaTotal As Double, ByRef dStoreTotal As Double, ByRef dMasterTotal As Double) As Double
    ...
    Query = " SELECT SUM(Price) AS TotalPrice FROM TB_º¸°ü¸®½ºÆ® "
    Query = Query & " WHERE SUBSTRING(InputDate,1,10) = '" & sDate & "' "
    Set Rs = New ADODB.Recordset
    Rs.Open Query, ADOCon, adOpenStatic, adLockOptimistic
...
'    Query = "SELECT DISTINCT P.Á¢¼öÀÏÀÚ, M.ÈÞ´ëÆù, (M.ÀüÈ­¹øÈ£+'-'+ M.ÀüÈ­2)  AS ÀüÈ­¹øÈ£ , M.¼º¸í, P.Ç°¸í, "
'    Query = Query & " P.ÅÃ¹øÈ£, P.»ö»ó, P.³»¿ë, P.±Ý¾×, P.°áÁ¦¿©ºÎ, P.»óÇ¥ "
'    Query = Query & " FROM TB_°í°´Á¤º¸ AS M, ÀÔÃâ°í AS P "
'    Query = Query & " WHERE (P.Á¢¼öÀÏÀÚ BETWEEN '" & Format(DTPicker1(0).Value, "YYYY-MM-DD") & "' "
'    Query = Query & " AND '" & Format(DTPicker1(1).Value, "YYYY-MM-DD") & "') "
'    Query = Query & " AND   (M.°í°´ÄÚµå = P.°í°´ÄÚµå AND P.È®ÀÎ <> 'È®') "
'    Query = Query & " AND   (P.ÆÇ¸ÅÃë¼Ò <> 'Y') "
'    Query = Query & " AND   (P.ÄÚµå LIKE '" & Left(cboGroup.Text, 1) & "%') "
'    Query = Query & " ORDER BY P.Á¢¼öÀÏÀÚ,  P.ÅÃ¹øÈ£ "
...
'        PrintText 150, (LineCnt * DefLineSpage) + DefPointTop, ObjRSet.Fields("»óÇ¥")
        ...
        PrintText 0, (LineCnt * DefLineSpage) + DefPointTop, Format(TotProssCnt, "@@@@")
        PrintText 10, (LineCnt * DefLineSpage) + DefPointTop, Format(ObjRSet.Fields("Á¢¼öÀÏÀÚ"), "YYYY-MM-DD")
        PrintText 30, (LineCnt * DefLineSpage) + DefPointTop, ObjRSet.Fields("ÅÃ¹øÈ£")
...
'    Query = "SELECT DISTINCT P.¼¼Å¹ºñÈ¯ºÒÀÏÀÚ, P.Á¢¼öÀÏÀÚ, M.ÈÞ´ëÆù, (M.ÀüÈ­¹øÈ£+'-'+ M.ÀüÈ­2)  AS ÀüÈ­¹øÈ£ , M.¼º¸í, P.Ç°¸í, "
'    Query = Query & " P.ÅÃ¹øÈ£, P.»ö»ó, P.³»¿ë, P.±Ý¾×, P.°áÁ¦¿©ºÎ, P.»óÇ¥ "
'    Query = Query & " FROM TB_°í°´Á¤º¸ AS M, ÀÔÃâ°í AS P "
'    Query = Query & " WHERE (LEFT(P.¼¼Å¹ºñÈ¯ºÒÀÏÀÚ,8) BETWEEN '" & Format(DTPicker1(0).Value, "YYYY-MM-DD") & "' "
'    Query = Query & " AND '" & Format(DTPicker1(1).Value, "YYYY-MM-DD") & "') "
...
Print_Start:
    ...
    Do Until ObjRSet.EOF
        ...
        PrintText 0, (LineCnt * DefLineSpage) + DefPointTop, Format(TotProssCnt, "@@@@")
        PrintText 10, (LineCnt * DefLineSpage) + DefPointTop, Format(Left(ObjRSet.Fields("¼¼Å¹ºñÈ¯ºÒÀÏÀÚ"), 8), "YYYY-MM-DD")
