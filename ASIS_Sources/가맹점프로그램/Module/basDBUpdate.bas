...
'    Query = "SELECT   dbo.F_StoreToCompanyInfo (가맹점코드,'','CODE') as 지사코드 "
    ...
    If Not ADORs.EOF Then
        ...
        Query = Query & ", 전산사용료     = '" & Trim(ADORs!전산사용료) & "'"
...
'-------------------------------------------------------------------
' 16) 가맹점의류 인터넷 자료 다운로드중...
'-------------------------------------------------------------------
    ' 이전수신일자부터 어제까지의 내용을 다운로드 한다.
    ' 등록 시간에 따라 수신이 되지 않는 문제가 발생할 수 있어 어제까지만 다운로드 한다.
    frmSplash.lblMsg.Caption = "가맹점 인터넷 자료 다운로드중..."
    frmSplash.lblCount.Caption = ""
    DoEvents

    Query = "SELECT   *"
    Query = Query & " FROM tb_가맹점의류인터넷 "
    Query = Query & " WHERE 가맹점코드 = '000000' "
    Set ADORs = New ADODB.RecordSet
    ADORs.Open Query, HostCon, adOpenForwardOnly, adLockReadOnly
        
    i = 0
    Query = "DELETE FROM TB_의류_인터넷"
    ADOCon.Execute Query
    Do Until ADORs.EOF
        i = i + 1
        ...
        SUBRs!의류코드 = ADORs!의류코드 & ""
        SUBRs!의류명 = ADORs!의류명 & ""
        SUBRs!금액 = ADORs!금액 & ""
        SUBRs!적용일자 = ADORs!적용일자
