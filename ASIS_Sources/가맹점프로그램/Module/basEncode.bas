...
Public Function Get_Decrypt(ByVal sInput As String, sPASSWORD As String) As String
    ...
    For i = 1 To Len(sInput)
        c = Mid(sInput, i, 1)                     ' Get character(i)
        k = InStr(1, sWHEEL1, c, vbBinaryCompare) ' Find character(i) on the first wheel:
        
        If k > 0 Then
            sResult = sResult & Mid(sWHEEL2, k, 1) ' Get the character with that index from the second wheel, and add it to result:
        Else
            sResult = sResult & c ' not found on the wheel, leave as it is:
        End If
        
        sWHEEL1 = LeftShift(sWHEEL1)  ' Rotate first wheel to the left:
        sWHEEL2 = RightShift(sWHEEL2) ' Rotate second wheel to the right:
    Next i
...
Public Function Set_Encrypt(ByVal sInput As String, sPASSWORD As String) As String
    Dim sWHEEL1 As String
    Dim sWHEEL2 As String
