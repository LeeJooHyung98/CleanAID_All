   ...
   Begin Threed.SSPanel SSPanel3 
      ...
      Begin FPSpreadADO.fpSpread fpSpread1 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         SpreadDesigner  =   "frmSago.frx":0000
      End
      Begin VB.TextBox txtInput 
         BeginProperty Font 
            Name            =   "굴림체"
...
Dim lDBCount As Long
    ...
    Next i
    ' 일자를 확인한다.
    strDate(0) = Format(dtInput(0).Value, "yyyymmdd")
    strDate(1) = Format(dtInput(1).Value, "yyyymmdd")
    strDate(3) = Format(dtInput(3).Value, "yyyymmdd")
    ...
    If iReadRecord = 0 Then
        ' 신규 입력일 경우
        ' 저장
        StrSQL = "SELECT  일련번호 FROM 사고품 ORDER BY 일련번호 DESC"
        Set MySet = MyDB.OpenRecordset(StrSQL)
        If MySet.RecordCount <= 0 Then
            lDBCount = 1
        Else
            lDBCount = CDbl(MySet!일련번호) + 1
        End If
        MySet.Close
        
        StrSQL = "INSERT INTO 사고품(일련번호, 접수일, 성명, 고객전화, 주소, 휴대폰, "
        StrSQL = StrSQL & " 품명, 상표, 구입일자, 색상, 구입처, 최초택번호, 최종택번호, 구입형태, 최초입고일, 최종입고일, "
        StrSQL = StrSQL & " 구입가격, 사고접수일, 사고종류, 사고내용, 사고의견, 보상금액, 합의금액, 처리유무 ) "
        StrSQL = StrSQL & "VALUES ('" & lDBCount & "', "
        StrSQL = StrSQL & "'" & Trim(strDate(0)) & "', "
        StrSQL = StrSQL & "'" & Trim(MaskEdBox1(1).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(2).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(1).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(3).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(4).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(5).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(strDate(1)) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(6).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(7).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(8).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(9).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(10).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(11).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(12).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(Replace(txtInput(13).Text, ",", "")) & "', "
        StrSQL = StrSQL & "'" & Trim(strDate(3)) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(14).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(15).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(16).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(17).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(txtInput(18).Text) & "', "
        StrSQL = StrSQL & "'" & Trim(Combo1.Text) & "') "
    ...
    ElseIf iEditMode = 2 Then
        ' 수정일 경우
        StrSQL = "UPDATE  사고품 SET "
        StrSQL = StrSQL & " 접수일 = '" & Trim(strDate(0)) & "', "
        StrSQL = StrSQL & " 성명 = '" & Trim(MaskEdBox1(1).Text) & "', "
        StrSQL = StrSQL & " 고객전화 = '" & Trim(txtInput(2).Text) & "', "
        StrSQL = StrSQL & " 주소 = '" & Trim(txtInput(1).Text) & "', "
        StrSQL = StrSQL & " 휴대폰 = '" & Trim(txtInput(3).Text) & "', "
        StrSQL = StrSQL & " 품명 = '" & Trim(txtInput(4).Text) & "', "
        StrSQL = StrSQL & " 상표 = '" & Trim(txtInput(5).Text) & "', "
        StrSQL = StrSQL & " 구입일자 = '" & Trim(strDate(1)) & "', "
        StrSQL = StrSQL & " 색상 = '" & Trim(txtInput(6).Text) & "', "
        StrSQL = StrSQL & " 구입처 = '" & Trim(txtInput(7).Text) & "', "
        StrSQL = StrSQL & " 최초택번호 = '" & Trim(txtInput(8).Text) & "', "
        StrSQL = StrSQL & " 최종택번호 = '" & Trim(txtInput(9).Text) & "', "
        StrSQL = StrSQL & " 구입형태 = '" & Trim(txtInput(10).Text) & "', "
        StrSQL = StrSQL & " 최초입고일 = '" & Trim(txtInput(11).Text) & "', "
        StrSQL = StrSQL & " 최종입고일 = '" & Trim(txtInput(12).Text) & "', "
        StrSQL = StrSQL & " 구입가격 = '" & Trim(txtInput(13).Text) & "', "
        StrSQL = StrSQL & " 사고접수일 = '" & Trim(strDate(3)) & "', "
        StrSQL = StrSQL & " 사고종류 = '" & Trim(txtInput(14).Text) & "', "
        StrSQL = StrSQL & " 사고내용 = '" & Trim(txtInput(15).Text) & "', "
        StrSQL = StrSQL & " 사고의견 = '" & Trim(txtInput(16).Text) & "',  "
        StrSQL = StrSQL & " 보상금액 = '" & Trim(txtInput(17).Text) & "',  "
        StrSQL = StrSQL & " 합의금액 = '" & Trim(txtInput(18).Text) & "',  "
        StrSQL = StrSQL & " 처리유무 = '" & Trim(Combo1.Text) & "'  "
        StrSQL = StrSQL & " WHERE 일련번호 = " & iReadRecord & ""
...
'+------------------------------------------------------
    ...
    StrSQL = " SELECT * FROM 사고품 "
    Set MySet = MyDB.OpenRecordset(StrSQL)
    
    If Not MySet.BOF Then MySet.MoveFirst
    ...
    With fpSpread1
        ...
        .MaxRows = 0
        .Row = -1
        .MaxCols = MySet.Fields.Count

        For i = 1 To MySet.Fields.Count
            .Col = i
            .ColWidth(i) = 10
            ...
            Select Case i
                ...
                Case 20
                    ...
                    .TypeHAlign = TypeHAlignLeft
            End Select
            .SetText i, 0, MySet.Fields.Item(i - 1).Name
        Next i
...
'+------------------------------------------------------
    ...
    StrSQL = " SELECT * FROM 사고품 WHERE "
    
    If Len(txtInput(30).Text) > 0 Then
        StrSQL = StrSQL & " 성명 LIKE '%" & txtInput(30).Text & "%'  AND  "
    ElseIf Len(txtInput(31).Text) > 0 Then
        StrSQL = StrSQL & " 고객전화 LIKE '%" & txtInput(31).Text & "%'  AND  "
    End If
    
    StrSQL = StrSQL & " ( 접수일 >= '" & Format(dtInput(4).Value, "yyyymmdd") & "' AND 접수일 <= '" & Format(dtInput(0).Value, "yyyymmdd") & "') "
    StrSQL = StrSQL & " ORDER BY 일련번호 "
    Set MySet = MyDB.OpenRecordset(StrSQL)
    
    
    If Not MySet.BOF Then MySet.MoveLast
    If MySet.RecordCount <= 0 Then
    ...
    With fpSpread1
        ...
        For i = 1 To MySet.RecordCount
            If .MaxRows < MySet.RecordCount Then
                .MaxRows = .MaxRows + 1
                .Row = .MaxRows
            ...
            For j = 1 To MySet.Fields.Count
                If j = 2 Or j = 9 Or j = 18 Then
                    .SetText j, i, Format(Trim(MySet.Fields(j - 1)), "0000-00-00")
                ElseIf j = 17 Or j = 22 Or j = 23 Then
                    .SetText j, i, Format(Trim(MySet.Fields(j - 1)), "#,###")
                Else
                .SetText j, i, Trim(MySet.Fields(j - 1))
                End If
            Next j
            .Row = .Row + 1
            MySet.MoveNext
...
'+------------------------------------------------------
    ...
    StrSQL = " SELECT * FROM 사고품 WHERE 일련번호 = " & strCode & ""
    Set MySet = MyDB.OpenRecordset(StrSQL)
    
    If MySet.BOF Or MySet.EOF Then
        Exit Sub
    End If
    
    iReadRecord = CInt(strCode)
    
    MaskEdBox1(1).Text = MySet!성명
    txtInput(1).Text = MySet!주소 & ""
    txtInput(2).Text = MySet!고객전화 & ""
    txtInput(3).Text = MySet!휴대폰 & ""
    txtInput(4).Text = MySet!품명 & ""
    txtInput(5).Text = MySet!상표 & ""
    If IsDate(Format(MySet!구입일자, "0000-00-00")) = True Then
        dtInput(1).Value = Format(MySet!구입일자, "0000-00-00")
    End If
    txtInput(6).Text = MySet!색상 & ""
    txtInput(7).Text = MySet!구입처 & ""
    txtInput(8).Text = MySet!최초택번호 & ""
    txtInput(9).Text = MySet!최종택번호 & ""
    txtInput(10).Text = MySet!구입형태 & ""
    txtInput(11).Text = MySet!최초입고일 & ""
    txtInput(12).Text = MySet!최종입고일 & ""
    txtInput(13).Text = MySet!구입가격 & ""
    dtInput(3).Value = Format(MySet!사고접수일 & "", "0000-00-00")
    txtInput(14).Text = MySet!사고종류 & ""
    txtInput(15).Text = MySet!사고내용 & ""
    txtInput(16).Text = MySet!사고의견 & ""
    txtInput(17).Text = MySet!보상금액 & ""
    txtInput(18).Text = MySet!합의금액 & ""
    Combo1.Text = MySet!처리유무 & ""
...
Private Sub SSPanel2_Click(Index As Integer)
    ...
    If Index = 17 Then
        ...
        Call SendTable_사고품(sSendData, MyHost, frmMain.ProgressBar1)
    End If
