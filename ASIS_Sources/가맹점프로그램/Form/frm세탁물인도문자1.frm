...
Begin VB.Form frm세탁물인도문자1 
   ...
   Begin Threed.SSPanel pnlDetView 
      ...
      Begin FPSpreadADO.fpSpread fpSpread1 
         ...
         Protect         =   0   'False
         ScrollBars      =   2
         SpreadDesigner  =   "frm세탁물인도문자1.frx":0000
         VisibleCols     =   2
         VisibleRows     =   50
         AppearanceStyle =   0
      End
   ...
   Begin FPSpreadADO.fpSpread SS 
      ...
      Protect         =   0   'False
      ScrollBars      =   2
      SpreadDesigner  =   "frm세탁물인도문자1.frx":0635
      VisibleCols     =   2
      VisibleRows     =   50
      AppearanceStyle =   0
   End
   Begin Threed.SSPanel SSPanel4 
      ...
      Begin VB.ComboBox cboSendText 
         ...
         EndProperty
         Height          =   405
         ItemData        =   "frm세탁물인도문자1.frx":0DDC
         Left            =   1530
         List            =   "frm세탁물인도문자1.frx":0DDE
         Style           =   2  '드롭다운 목록
         TabIndex        =   46
         Top             =   600
         Width           =   8895
        ...
        Case 0
            For lRow = 1 To SS.MaxRows
                ...
                If SS.Text <> "" Then
                    ...
                    ' 해당 회원의 정보를 얻어온다.
                    If Get_고객정보(SS.Text) <> "Error" Then
...
'        txtServer(0).Text = Trim(RS01.Fields("SMS_IP") & "")
'        txtServer(1).Text = Trim(RS01.Fields("SMS_DB") & "")
'        txtServer(2).Text = Trim(RS01.Fields("SMS_ID") & "")
'        txtServer(3).Text = Trim(RS01.Fields("SMS_PWD") & "")
...
Private Sub DataDisplay()
    ...
    If ADORs.RecordCount > 0 Then
        ...
        While Not ADORs.EOF
            ...
            Query = Query & " WHERE 접수일자 >= '" & Format(DTPicker1.Value, "YYYY-MM-DD") & "' "
            Query = Query & "   AND 접수일자 <= '" & Format(DTPicker2.Value, "YYYY-MM-DD") & "' "
            Query = Query & "   AND NOT ( 지사출고상태 = '出' OR 지사출고상태 = '反') "
            ...
            If SUBRs.EOF Then
                ...
                ' 해당 회원의 정보를 얻어온다.
                If Get_고객정보(ADORs.Fields("고객코드") & "") <> "Error" Then
                    ...
                    SS.SetText 4, lRow, 고객정보.휴대전화
                                        
                    '-----------------------------------------------------------------------------------
                    ' 미출고된 내역의 정보를 구한다. 시작텍과 종료텍의 수량을 구한다.
                    '-----------------------------------------------------------------------------------
                    Query = " SELECT Min(택번호) As 시작번호, Max(택번호) AS 종료번호, Count(택번호) AS 수량,"
                    Query = Query & " MAX(지사출고일자) AS 본사출고일 FROM TB_입출고 "
                    Query = Query & " WHERE 접수일자 >= '" & Format(DTPicker1.Value, "YYYY-MM-DD") & "' "
                    Query = Query & "   AND 접수일자 <= '" & Format(DTPicker2.Value, "YYYY-MM-DD") & "' "
                    Query = Query & "   AND (지사출고상태 = '出'  OR 지사출고상태 = '反') "     ' 본사 출고 상태인것
                    ...
                    If SUBRs.EOF = False Then
                        ...
                        If Trim(고객정보.휴대전화) = "" Then
                            SS.SetText 9, lRow, ""
                        Else
                            SS.SetText 9, lRow, GetLastUseDate(고객정보.휴대전화) '-- 시간이 너무 걸리네 ㅡㅡ
                        End If
                    ...
                    Query = Query & " WHERE 접수일자 >= '" & Format(DTPicker1.Value, "YYYY-MM-DD") & "' "
                    Query = Query & "   AND 접수일자 <= '" & Format(DTPicker2.Value, "YYYY-MM-DD") & "' "
                    Query = Query & "   AND 지사출고상태 = '反' "     ' 본사 출고 상태인것
                    Query = Query & "   AND 확인 <> '확' "    ' 미출고 상태인것
                    Query = Query & "   AND 고객코드 = '" & ADORs.Fields("고객코드") & "" & "' "
                    Query = Query & "   AND 판매취소 <>  'Y' "
                    Set SUBRs = New ADODB.Recordset
...
Private Sub DisplayDetailData(ByVal sCustCode As String)
    ...
    While Not ADORs.EOF
        ...
        With fpSpread1
            ...
            .Col = 4:   .Text = Format(ADORs.Fields("지사출고일자") & "", "YYYY-MM-DD")
            .Col = 5:   .Text = ADORs.Fields("지사출고상태") & ""
