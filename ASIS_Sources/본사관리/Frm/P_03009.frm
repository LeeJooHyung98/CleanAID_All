   ...
   Begin SSSplitter.SSSplitter SSSplitter 
      ...
      Begin Threed.SSPanel panInput 
         ...
         Begin VB.ComboBox cboOffice 
            Height          =   315
            ItemData        =   "P_03009.frx":065C
            Left            =   1245
            List            =   "P_03009.frx":065E
            Locked          =   -1  'True
            TabIndex        =   21
            Text            =   "cboOffice"
            Top             =   60
      ...
      Begin Threed.SSPanel pnlHeader 
         ...
         PictureBackground=   "P_03009.frx":0BFA
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin Threed.SSPanel SSPanel 
         ...
         EndProperty
         PictureBackgroundStyle=   2
         PictureBackground=   "P_03009.frx":0DFC
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin FPSpreadADO.fpSpread spdView 
         ...
         MaxCols         =   13
         ScrollBars      =   2
         SpreadDesigner  =   "P_03009.frx":3CCE
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
      Begin FPSpreadADO.fpSpread spdView 
         ...
         MaxCols         =   4
         ScrollBars      =   2
         SpreadDesigner  =   "P_03009.frx":44ED
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
   End
End
Attribute VB_Name = "P_03009"
...
Private Sub Data_Display()
    ...
    sValue(1) = Format(dtInput(0).Value, "YYYY-MM-DD") + " 00:00:00"
    sValue(2) = Format(dtInput(1).Value, "YYYY-MM-DD") + " 23:59:59"
    ...
    Else
        Set RS01 = New ADODB.Recordset
        Set RS01 = ExecPro("SP_03009_03", sValue(), Err_Num, Err_Dec)
    End If
...
Private Sub Data_DisplayStore(sStoreCode As String)
    ...
    Else
        Set RS01 = New ADODB.Recordset
        Set RS01 = ExecPro("SP_03009_01", sValue(), Err_Num, Err_Dec)
    End If
    ...
    With spdView(1)
        ...
        Do Until RS01.EOF
            ...
            .Col = 1:  .Text = Format(RS01!TAG_NO, "000-00-0000") & "" ' 2
            .Col = 2:  .Text = RS01!접수일자 & ""                      ' 1
            .Col = 3:  .Text = RS01!지사출고일자 & ""                  ' 3
            .Col = 4:  .Text = RS01!출고차수 & ""                      ' 4
            .Col = 5:  .Text = RS01!의류코드 & ""                      ' 5
            .Col = 6:  .Text = RS01!의류명 & ""                        ' 6
            .Col = 7:  .Text = RS01!색상 & ""                          ' 7
            .Col = 8:  .Text = RS01!무늬 & ""                          ' 8
            .Col = 9:  .Text = RS01!내용 & ""                          ' 9
            .Col = 10: .Text = RS01!금액 & ""                          '10
            .Col = 11: .Text = RS01!상표 & ""                          '11
            .Col = 12: .Text = RS01!반품사유 & ""                      '12
...
Private Sub DataPrint()
    ...
    Open App.Path & "\XML\지사반품현황.XML" For Output As #FileNumber
    
    Print #FileNumber, "<?xml version=""1.0"" encoding=""EUC-KR""?>"
    Print #FileNumber, "<root>"
    
          XML = "    <조건>"
    XML = XML & "        <지사명>지 사 명 : " & Func_Replace(cboOffice.Text) & "</지사명>"
    XML = XML & "        <가맹점>가 맹 점 : " & Func_Replace(cboInput.Text) & "</가맹점>"
    XML = XML & "        <출고일자>출고일자 : " & Format(dtInput(0).Value, "YYYY-MM-DD") & " ~ " & Format(dtInput(1).Value, "YYYY-MM-DD") & "</출고일자>"
    XML = XML & "        <반품수량>반품수량 : " & txtNum(0).Text & "</반품수량>"
    XML = XML & "        <반품금액>반품금액 : " & txtNum(1).Text & "</반품금액>"
    XML = XML & "   </조건>"
    ...
    With spdView(1)
        For i = 1 To .MaxRows
            .Row = i
                             ...
                             XML = "    <Data>"
            .Col = 1:  XML = XML & "        <택번호>" & Func_Replace(.Text) & "</택번호>"
            .Col = 2:  XML = XML & "        <접수일자>" & Func_Replace(.Text) & "</접수일자>"
            .Col = 3:  XML = XML & "        <지사출고일자>" & Func_Replace(.Text) & "</지사출고일자>"
            .Col = 4:  XML = XML & "        <출고차수>" & Func_Replace(.Text) & "</출고차수>"
            .Col = 5:  XML = XML & "        <의류코드>" & .Text & "</의류코드>"
            .Col = 6:  XML = XML & "        <의류명>" & .Text & "</의류명>"
            .Col = 7:  XML = XML & "        <색상>" & .Text & "</색상>"
            .Col = 8:  XML = XML & "        <무늬>" & .Text & "</무늬>"
            .Col = 9:  XML = XML & "        <내용>" & .Text & "</내용>"
            .Col = 10: XML = XML & "        <금액>" & .Text & "</금액>"
            .Col = 11: XML = XML & "        <상표>" & .Text & "</상표>"
                       XML = XML & "   </Data>"
            Print #FileNumber, XML
        Next i
    ...
    With rpt지사반품현황
        .dc.FileURL = App.Path & "\XML\지사반품현황.XML"
        .PrintReport False
    ...
    With spdView(1)
        For i = 1 To .MaxRows
            ...
            If .Value = True Then
                ...
                .Col = 3:    sValue(0) = .Text   '반품일자
                .Col = 4:    sValue(1) = .Text   '차수
                             sValue(2) = .Tag    '가맹점코드
                .Col = 1:    sValue(3) = Replace(.Text, "-", "") '택번호
                .Col = 2:    sValue(4) = .Text   '접수일자
                .Col = 12:   sValue(5) = .Text   '반품사유
                ...
                Else
                    Set RS01 = New ADODB.Recordset
                    Set RS01 = ExecPro("SP_03009_02", sValue(), Err_Num, Err_Dec)
                End If
...
Private Sub spdView_Change(Index As Integer, ByVal Col As Long, ByVal Row As Long)
    If Index = 1 Then spdView(Index).SetText 13, Row, "1"
End Sub
