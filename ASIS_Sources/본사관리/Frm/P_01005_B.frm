...
Begin VB.Form P_01005_B 
   Caption         =   "가맹점별 요일할인 등록"
   ClientHeight    =   12165
   ClientLeft      =   6765
   ClientTop       =   3345
   ClientWidth     =   15885
   ...
   Begin SSSplitter.SSSplitter SSSplitter 
         ...
         Begin XtremeSuiteControls.TabControlPage TabControlPage 
            ...
            Begin SSSplitter.SSSplitter SSSplitter2 
               ...
               Begin FPSpreadADO.fpSpread spdView 
                  ...
                  MaxCols         =   6
                  ScrollBars      =   2
                  SpreadDesigner  =   "P_01005_B.frx":070E
                  Appearance      =   1
                  HighlightHeaders=   1
                  HighlightStyle  =   1
               End
               Begin FPSpreadADO.fpSpread spdList 
                  ...
                  Protect         =   0   'False
                  ScrollBars      =   2
                  SpreadDesigner  =   "P_01005_B.frx":0D5D
                  HighlightHeaders=   1
                  HighlightStyle  =   1
               End
            End
         ...
         Begin XtremeSuiteControls.TabControlPage TabControlPage 
            ...
            Begin SSSplitter.SSSplitter SSSplitter1 
               ...
               Begin FPSpreadADO.fpSpread spdCloth 
                  ...
                  GridSolid       =   0   'False
                  MaxCols         =   6
                  SpreadDesigner  =   "P_01005_B.frx":13F5
                  Appearance      =   1
                  HighlightHeaders=   1
                  HighlightStyle  =   1
               End
      ...
      Begin Threed.SSPanel panSub 
         ...
         Begin VB.ComboBox cboWeek 
            Height          =   315
            ItemData        =   "P_01005_B.frx":20C5
            Left            =   975
            List            =   "P_01005_B.frx":20C7
            Style           =   2  '드롭다운 목록
            TabIndex        =   24
            Top             =   2235
            Width           =   3015
      ...
      Begin Threed.SSPanel pnlHeader 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Caption         =   " 가맹점별 요일할인 등록 (P_01005_B)"
         PictureBackgroundStyle=   2
         PictureBackground=   "P_01005_B.frx":2ADB
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin Threed.SSPanel SSPanel 
         ...
         EndProperty
         PictureBackgroundStyle=   2
         PictureBackground=   "P_01005_B.frx":2CDD
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin FPSpreadADO.fpSpread sprMaster 
         ...
         MaxCols         =   3
         ScrollBars      =   2
         SpreadDesigner  =   "P_01005_B.frx":5BAF
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
      Begin FPSpreadADO.fpSpread sprList 
         ...
         MaxCols         =   5
         ScrollBars      =   2
         SpreadDesigner  =   "P_01005_B.frx":6146
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
      Begin FPSpreadADO.fpSpread spdClass 
         ...
         MaxCols         =   3
         ScrollBars      =   2
         SpreadDesigner  =   "P_01005_B.frx":6774
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
   End
End
Attribute VB_Name = "P_01005_B"
...
Private Sub SPR_Resize()
    ...
    spdView(0).Width = Me.Width - 9500
    spdView(0).Height = Me.Height - 3140
...
'            Dim i As Integer
'
'            spdClass.MaxCols = RS01.Fields.Count
'            spdClass.MaxRows = RS01.RecordCount
'
'            'Call spdDisplay3(RS01)
'            Call fpSpread_Display(spdClass, RS01)
'            Call GetColWidth(REG_App, Me.Name & "A2", spdClass)
'        End If
'
'    End If
End Sub
...
Private Sub Data_Display()
    ...
    Set RS01 = New ADODB.Recordset
    Set RS01 = ExecPro("SP_01005_B_00", sValue(), Err_Num, Err_Dec)
...
Public Sub DataSave()
    ...
    If cboWeek.ListIndex < 0 Then
        MsgBox "요일을 선택하세요.", vbInformation, "확인"
        
        cboWeek.SetFocus
        Exit Sub
    End If
    
    If spdCloth.MaxRows = 0 Then
        MsgBox "할인계산 버튼을 클릭한 후 저장하세요", vbInformation, "확인"
        Exit Sub
    End If
    ...
    For iRow = 1 To sprList.MaxRows
        ...
        If sprList.Text = "1" Then
            ...
            '-------------------------------------------------------------------
            ' TB_가맹점요일할인 삭제 - SP_01005_B_01
            '-------------------------------------------------------------------
            ReDim sValue(3)
            
            sValue(0) = 가맹점코드                             ' 0
            sValue(1) = Format(dtInput(0).Value, "YYYY-MM-DD") ' 1
            sValue(2) = Format(dtInput(1).Value, "YYYY-MM-DD") ' 2
            sValue(3) = Mid(cboWeek.Text, 2, 1)                ' 3 할인요일
            
            Call ExecPro("SP_01005_B_01", sValue(), Err_Num, Err_Dec)
            ...
            Do Until RS01.EOF
               ...
               '-------------------------------------------------------------------
                ...
                sValue(3) = Mid(cboWeek.Text, 2, 1)                ' 3 할인요일
                sValue(4) = Trim(RS01!의류코드) & ""               ' 4 의류코드
                sValue(5) = Trim(RS01!의류명) & ""                 ' 5 의류명
                sValue(6) = RS01!금액                              ' 6 정상가격
                sValue(7) = 할인율                                 ' 7 할인률
                ...
                Call ExecPro("SP_01005_B_05", sValue(), Err_Num, Err_Dec)
...
Public Sub DataDelete()
    Rtn = MsgBox("해당되는 데이터를 삭제하시겠습니까?", vbYesNo + vbQuestion + vbDefaultButton2, "삭제")
...
Private Sub spdView_ButtonClicked(Index As Integer, ByVal Col As Long, ByVal Row As Long, ByVal ButtonDown As Integer)
    On Error GoTo ERR_RTN
    
    If Index = 0 And Col = 1 Then
        Dim vText   As Variant
        ReDim sValue(4)
                                                
        spdView(0).GetText 2, Row, vText:       sValue(0) = CStr(vText)
        spdView(0).GetText 4, Row, vText:       sValue(1) = CStr(vText)
        spdView(0).GetText 5, Row, vText:       sValue(2) = CStr(vText)
        spdView(0).GetText 6, Row, vText
            Select Case CStr(vText)
                Case "일":  sValue(3) = "1"
                Case "월":  sValue(3) = "2"
                Case "화":  sValue(3) = "3"
                Case "수":  sValue(3) = "4"
                ...
                Case "토":  sValue(3) = "7"
            End Select
        spdView(0).GetText 1, Row, vText:      sValue(4) = IIf(CStr(vText) = "1", "Y", "N")
    
        Call ExecPro("SP_01004_A_23", sValue(), Err_Num, Err_Dec)
...
Private Sub Data_Display2(가맹점코드 As String)
    ...
    Set RS01 = New ADODB.Recordset
    Set RS01 = ExecPro("SP_01005_A_00", sValue(), Err_Num, Err_Dec)
