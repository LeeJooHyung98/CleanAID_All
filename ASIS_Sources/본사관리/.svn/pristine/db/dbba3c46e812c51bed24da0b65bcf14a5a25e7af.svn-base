...
Public Function Fnc_TcpConnect(tcpHost As Winsock) As Boolean
    ...
    If tcpHost.State <> sckConnected Then
        ...
        ' 클라이언트에서 서버에 연결을 시도함
        tcpHost.RemoteHost = Trim(GetIniStr("Store Server", "ServerNameOrIP", "", m_iniFile))
        tcpHost.RemotePort = Val(Trim(GetIniStr("Store Server", "MessagePort", "", m_iniFile)))
        tcpHost.Connect
...
Public Function Fnc_TcpSendMessage(tcpHost As Winsock, sSendMessage As String) As Boolean
    ...
    If tcpHost.State = sckConnected Then
        ...
        DoEvents
        Call PanelsMsg("보낸 메시지 : " & sSendMessage)
