   ...
   Begin SSSplitter.SSSplitter SSSplitter 
      ...
      Begin Threed.SSPanel pnlHeader 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Caption         =   " 일일출고 매장별 현황 (P_03016)"
         PictureBackgroundStyle=   2
         PictureBackground=   "P_03016.frx":3647
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin Threed.SSPanel SSPanel 
         ...
         EndProperty
         PictureBackgroundStyle=   2
         PictureBackground=   "P_03016.frx":3849
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin FPSpreadADO.fpSpread spdView 
         ...
         MaxRows         =   34
         ScrollBars      =   2
         SpreadDesigner  =   "P_03016.frx":671B
         UserResize      =   1
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
      Begin FPSpreadADO.fpSpread spdView1 
         ...
         MaxCols         =   23
         Protect         =   0   'False
         SpreadDesigner  =   "P_03016.frx":6D73
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
      Begin Threed.SSPanel SSPanel 
...
Private Sub Data_Display()
    ...
    If HeadOffice = MASTER_OFFICE_CODE Then
        ...
        Set RS01 = New ADODB.Recordset
        Set RS01 = ExecProMaster("SP_03016_00", sValue(), Err_Num, Err_Dec)
    Else
        Set RS01 = New ADODB.Recordset
        Set RS01 = ExecPro("SP_03016_00", sValue(), Err_Num, Err_Dec)
    End If
...
Private Sub Data_Display2()
    ...
    For nRow = 0 To spdView.MaxRows
        spdView.Col = 5:  spdView.Row = nRow
        ' 매장코드만 적용한다.
        If spdView.Text = "1" Then
            spdView.Col = 1
            If IsNumeric(spdView.Text) Then
                        
                SSQL2 = SSQL2 & "'" & Trim(spdView.Text) & "', "
                sValue(2) = Trim(spdView.Text)
                ...
                If HeadOffice = MASTER_OFFICE_CODE Then
                    ...
                    Set RS01 = New ADODB.Recordset
                    Set RS01 = ExecProMaster("SP_03016_01", sValue(), Err_Num, Err_Dec)
                Else
                    Set RS01 = New ADODB.Recordset
                    Set RS01 = ExecPro("SP_03016_01", sValue(), Err_Num, Err_Dec)
                End If
...
Private Sub DataPrint()
    ...
    For i = 1 To spdView.MaxRows
        ...
        If Trim(spdView.Text) = "" Then Exit For
        출고회자 = 출고회자 & Trim(spdView.Text) & ","
    
    Next i
    If Len(출고회자) > 2 Then 출고회자 = Mid(출고회자, 1, Len(출고회자) - 1)                   ' 마지막 ,을 제거한다.
    ...
    Print #FileNumber, "<root>"
          ...
          XML = "    <조건>"
    XML = XML & "        <가맹점>" & Func_Replace(Mid(cboInput.Text, 10)) & " (" & 택번호 & ")  출고내역</가맹점>"
    XML = XML & "        <출고수량>출고수량 : " & spdView1.MaxRows & " 점</출고수량>"
    XML = XML & "        <출고일자>출고일자 : " & Format(dtInput.Value, "YYYY년 MM월 DD일") & "  (출고회차 : " & 출고회자 & ")</출고일자>"
