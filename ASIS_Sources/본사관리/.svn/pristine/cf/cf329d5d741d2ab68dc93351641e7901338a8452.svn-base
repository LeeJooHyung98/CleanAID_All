...
'Public Const m_UpdateUrl      As String = "www.clean-aid.co.kr:8090/cleanaid"
Public Const m_UpdateUrl      As String = "www.insoftnet.com/upgrade/cleanaid"
...
Public Function GetDefaultValues() As Boolean
    ...
    m_iniFile = App.Path & "\CleanAid.Ini"
    ...
    Store.Code = Get_Decrypt(GetIniStr("Store DATA", "StoreCode", "", m_iniFile), "")
    Store.Name = Get_Decrypt(GetIniStr("Store DATA", "StoreName", "", m_iniFile), "")
...
'    iTimeout = Val(Trim(GetIniStr("Store Server", "CommandTimeout", "", m_iniFile)))
'    DBPort = Trim(GetIniStr("Store Server", "MessagePort", "", m_iniFile))
    ...
    MSTCon.ConnectionString = "Provider=SQLOLEDB;Persist Security Info=False;User ID=" & DBUserID & ";Password=" & DBUserPwd & ";Initial Catalog=" & "LAUNDRY" & strDatabase & ";Data Source=" & DBServer & "," & DBPort
    MSTCon.CommandTimeout = DBTimeOut
    MSTCon.CursorLocation = adUseClient
    MSTCon.Open
...
'Return Value ; True  : DB Open
'             ; False : DB Open 실패
Public Function DBOpen_Laundry() As Boolean
    ...
    ADOCon.CommandTimeout = Val(DBTimeOut)
...
'Return Value ; True  : DB Open
'             ; False : DB Open 실패
Public Function DBOpen_EJangbu(AdoConJub As ADODB.Connection) As Boolean
    
    On Error GoTo DBOpenError
 
    AdoConJub.ConnectionString = "Provider=SQLOLEDB;Persist Security Info=False;User ID=" & DBUserID & ";Password=" & DBUserPwd & ";Initial Catalog=" & "EJANGBU" & ";Data Source=" & DBServer & "," & DBPort
    AdoConJub.CommandTimeout = Val(DBTimeOut)
    AdoConJub.CursorLocation = adUseClient    '이문장이 있어야 스프리드에 보여짐... 나머지는 잘모름
    AdoConJub.Open
...
'
'--------------------------------------------------------------------
Public Function fpSpread_Display(SS As Object, Rs As ADODB.Recordset, Optional bFlag As Boolean = True)
    Dim i As Long
    Dim j As Long
    ...
    With SS
        ...
        For i = 1 To Rs.RecordCount
            For j = 1 To Rs.Fields.Count
                .SetText j, i, Replace(Trim(IIf(IsNull(Rs.Fields(j - 1)), "", Rs.Fields(j - 1))), vbCrLf, " ") & ""
            Next j
