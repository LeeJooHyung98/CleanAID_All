...
Begin VB.Form P_04028 
   Caption         =   "매장 매출 현황(보고용-지사기준)"
   ClientHeight    =   12450
   ClientLeft      =   1200
   ClientTop       =   2085
   ClientWidth     =   16515
   ...
   Begin SSSplitter.SSSplitter SSSplitter 
      ...
      Begin FPSpreadADO.fpSpread spdView 
         ...
         MaxCols         =   5
         ScrollBars      =   2
         SpreadDesigner  =   "P_04028.frx":065C
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
      ...
      Begin Threed.SSPanel pnlHeader 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Caption         =   "매장 매출 현황(보고용-지사기준) (P_04027)"
         PictureBackgroundStyle=   2
         PictureBackground=   "P_04028.frx":11D8
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin Threed.SSPanel SSPanel 
         ...
         EndProperty
         PictureBackgroundStyle=   2
         PictureBackground=   "P_04028.frx":13DA
         BorderWidth     =   0
         BevelOuter      =   0
         Alignment       =   1
         RoundedCorners  =   0   'False
      ...
      Begin FPSpreadADO.fpSpread spdView2 
         ...
         GridSolid       =   0   'False
         MaxCols         =   13
         SpreadDesigner  =   "P_04028.frx":42AC
         HighlightHeaders=   1
         HighlightStyle  =   1
      End
      Begin Threed.SSPanel SSPanel 
...
Private Sub Form_Activate()
    ...
    If P_04028_Flag = False Then
        ...
        Set RS01 = ExecPro("SP_04028_00", sValue(), Err_Num, Err_Dec)
...
Private Sub Data_Display()
    ...
    With spdView2
        ...
        .Row = -999
        .Col = 5:  .Text = Right(Format(dtInput(0).Value, "yyyy"), 4) & "년"
        .Col = 7:  .Text = Right(Format(DateAdd("yyyy", -1, dtInput(0).Value), "yyyy"), 4) & "년"
        
        .Row = -998
        .Col = 11:  .Text = Right(Format(dtInput(0).Value, "yyyy"), 4) & "년"
        .Col = 12:  .Text = Right(Format(DateAdd("yyyy", -1, dtInput(0).Value), "yyyy"), 4) & "년"
        
        panCaption(2).Caption = Right(Format(dtInput(0).Value, "yyyy"), 4) & "년"
        panCaption(5).Caption = Right(Format(DateAdd("yyyy", -1, dtInput(0).Value), "yyyy"), 4) & "년"
        
    End With
    
    sKeyCode(0) = "":   sKeyCode(1) = ""
    dTemp(0) = 0:       dTemp(1) = 0:       dTemp(2) = 0:       dTemp(3) = 0
    dTempTotal(0) = 0:  dTempTotal(1) = 0:  dTempTotal(2) = 0:  dTempTotal(3) = 0
    ...
    For nRow = 0 To spdView.MaxRows
        ...
        If spdView.Text = "1" Then
            spdView.Col = 1
            If IsNumeric(Mid(spdView.Text, 2, 4)) Then '<- 매장만, 지사제외
                        
                spdView.Col = 1
                SSQL2 = SSQL2 & Mid(spdView.Text, 2, 4) & ","
            End If
        End If
    Next nRow
    ...
    sValue(3) = IIf(chkSelect(0).Value = xtpChecked, "Y", "N")
    sValue(4) = IIf(chkSelect(1).Value = xtpChecked, "Y", "N")
    sValue(5) = IIf(chkSelect(2).Value = xtpChecked, "Y", "N")
    sValue(6) = IIf(chkSelect(3).Value = xtpChecked, "Y", "N")
    sValue(7) = IIf(chkSelect(4).Value = xtpChecked, "Y", "N")
    sValue(8) = IIf(chkSelect2(0).Value = xtpChecked, "Y", "N")
    
    If sValue(0) = "" Then Exit Sub
    Screen.MousePointer = vbHourglass
    
    Set RS01 = New ADODB.Recordset
    Set RS01 = ExecPro("SP_04028_01", sValue(), Err_Num, Err_Dec)
    ...
    With spdView2
        ...
        Do Until RS01.EOF
            ...
            ' 지사별 합계 처리
            If sKeyCode(0) <> Trim(RS01!지사코드 & "") Then
                .MaxRows = .MaxRows + 1
                .Row = .MaxRows
                ...
                If dTemp(2) > 0 Then
                    .Col = 9:  .Text = Round((dTemp(0) - dTemp(2)) / dTemp(2) * 100, 1)
                ...
                dTempTotal(3) = dTempTotal(3) + dTemp(3)
            ...
            .Col = 6:  .Text = RS01!지사매출 & ""
            .Col = 7:  .Text = RS01!매출금액2 & ""
            .Col = 8:  .Text = RS01!지사매출2 & ""
            .Col = 10:  .Text = RS01!비고메모 & ""
            
            
            dTemp(0) = dTemp(0) + CDbl(RS01!매출금액 & "")
            dTemp(1) = dTemp(1) + CDbl(RS01!지사매출 & "")
            dTemp(2) = dTemp(2) + CDbl(RS01!매출금액2 & "")
            dTemp(3) = dTemp(3) + CDbl(RS01!지사매출2 & "")
            
            If IsNumeric(RS01!매출금액 & "") = True And IsNumeric(RS01!매출금액2 & "") Then
                If Val(Val(RS01!매출금액2 & "")) > 0 Then
                    .Col = 9:  .Text = Round((Val(RS01!매출금액 & "") - Val(RS01!매출금액2 & "")) / Val(RS01!매출금액2 & "") * 100, 1)
        ...
        txtNum(2).Value = dTempTotal(2)
        txtNum(3).Value = dTempTotal(3)
        
        ' 합계 전년 대비 매출 신장률
        If IsNumeric(txtNum(0).Value) = True And IsNumeric(txtNum(2).Value) Then
            If Val(txtNum(2).Value) > 0 Then
                txtNum(4).Value = Round((Val(txtNum(0).Value) - Val(txtNum(2).Value)) / Val(txtNum(2).Value) * 100, 1)
            
                txtNum(4).ForeColor = IIf(Val(txtNum(2).Value) < 0, vbRed, vbBlack)
    ...
    With spdView2
       For nRow = 1 To .DataRowCnt
           ...
           If sValue(0) <> "" Then
                ...
                If Not RS01.EOF Then .SetText 11, nRow, CVar(RS01!객수1 & "")
    ...
    With spdView2
       For nRow = 1 To .DataRowCnt
           ...
           If sValue(0) <> "" Then
                ...
                If Not RS01.EOF Then .SetText 12, nRow, CVar(RS01!객수1 & "")
        ...
        For nRow = 1 To .DataRowCnt
           ...
           ' 지사가 변경될 경우 합계를 출력한다.
           If sKeyCode <> CStr(vText) Then
                .SetText 11, nRow - 1, CVar(dTemp(0))
                .SetText 12, nRow - 1, CVar(dTemp(1))
                
                sKeyCode = CStr(vText)
                dTemp(0) = 0:   dTemp(1) = 0
            End If
            
           .GetText 11, nRow, vText:     dTemp(0) = dTemp(0) + CDbl(IIf(IsNumeric(vText) = False, 0, vText))
           .GetText 12, nRow, vText:    dTemp(1) = dTemp(1) + CDbl(IIf(IsNumeric(vText) = False, 0, vText))
...
'        .Col = 10:  .Formula = "SUM(J1:J" & .MaxRows - 1 & ")"
    ...
    With spdView2
       For nRow = 1 To .DataRowCnt
           ...
           .GetText 11, nRow, vText:     sValue(0) = CStr(vText)
           .GetText 12, nRow, vText:     sValue(1) = CStr(vText)
           
            If IsNumeric(sValue(0)) = True And IsNumeric(sValue(1)) Then
                If Val(sValue(1)) > 0 Then
                    .SetText 13, nRow, Round((Val(sValue(0)) - Val(sValue(1))) / Val(sValue(1)) * 100, 1)
