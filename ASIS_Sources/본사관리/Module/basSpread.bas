...
' Author    : pds2004
' Purpose   : 스프레드의 마지막 Row를 추가하고 합계를 출력한다.
'--------------------------------------------------------------------------------------------------------------
Public Function SpreadSum(spr As Object, ColTitle As Long, Col As Long) As Double
    Dim sRow    As String
    ...
    With spr
        ...
        If ColTitle > 0 Then
            ...
            If .MaxRows > 0 Then
                ...
                .Col = ColTitle: .Text = "합계"
            End If
        End If
            
        ' 합계를 처리한다.
        If Col >= 1 And Col <= 26 Then
            sRow = Chr(Col + 64)
        ElseIf Col >= 27 And Col <= 52 Then
            sRow = "A" & Chr((Col - 26) + 64)
        End If
        
        .Col = Col: .Formula = "SUM(" & sRow & "1:" & sRow & .MaxRows - 1 & ")"
        
        ' 정렬시 Formula식의 위치가 변경 되기 때문에 값을 계산후 그 값을 다시 넣어 준다.
        .GetText Col, .MaxRows, vText
        .SetText Col, .MaxRows, CStr(vText)
        .Col = Col: .Formula = ""
        
        SpreadSum = CDbl(vText)
